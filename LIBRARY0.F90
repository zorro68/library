...
REAL*16 FUNCTION QFACT(NUM)
    IMPLICIT NONE
    INTEGER*4 NUM,J
    QFACT=1.0Q0
    IF(NUM==0)RETURN
	DO J=1,NUM
		QFACT=QFACT*REAL(J,16)
	END DO
    RETURN
END
...
REAL*8 FUNCTION DFACTI(NUM)
    IMPLICIT NONE
    INTEGER*4 NUM,J
    IF(DMOD(DBLE(NUM),2.D0)==0.D0) STOP 'ERROR IN DFACTI: THE NUMBER IS NO AN ODD ONE'
    DFACTI=1.0D0 
    IF(NUM<=0)RETURN
	DO J=1,NUM,2
		DFACTI=DFACTI*DBLE(J)
	END DO
    RETURN
    END
    
REAL*16 FUNCTION QFACTI(NUM)
    IMPLICIT NONE
    INTEGER*4 NUM,J
    IF(QMOD(REAL(NUM,16),2.Q0)==0.Q0) STOP 'ERROR IN QFACTI: THE NUMBER IS NO AN ODD ONE'
    QFACTI=1.0Q0
    IF(NUM<=0)RETURN
	DO J=1,NUM,2
...
! ------------------------------------------------------------------------
!   FUNCTION TO CALCULATE THE FACTORIAL FUNCTION OF A INTEGER NUMBER (NUM)
! ------------------------------------------------------------------------
    
INTEGER*4 FUNCTION NFACT(NUM)
    IMPLICIT NONE
	INTEGER*4 NUM,J
